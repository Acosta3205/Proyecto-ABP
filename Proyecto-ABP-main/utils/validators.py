...
def EliminarReservaCliente(page, db, nombre, telefono):
    ...
    if len(errores) > 0:
        ...
        if selecionar_row_id is None:
            page.dialog = ft.AlertDialog(
                title=ft.Text("Seleccione una reserva"),
                content=ft.Text("Por favor, seleccione una reserva para eliminar."),
                actions=[ft.TextButton("Aceptar", on_click=lambda e: close_dialog(page, db))],
            )
            page.dialog.open = True
            page.update()
        else:
            if selecionar_row_id is not None:
                page.dialog = ft.AlertDialog(
                    title=ft.Text("Desea eliminar la reserva?"),
                    content=ft.Text("Esta accion no se puede deshacer."),
                    actions=[
                        ft.TextButton("Aceptar", on_click=lambda e: EliminarReservaConfirmada(page, db)),
                        ft.TextButton("Cancelar", on_click=lambda e: close_dialog(page, db))
                        ],
                )
                page.dialog.open = True
                page.update()

def EliminarReservaConfirmada(page, db):

    # Eliminar la reserva          
    EliminarReservaSelecionada(db)
